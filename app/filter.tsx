...
const Filter: React.FC<FilterProps> = ({
  ...
  const isDateDisabled = (date: Date) => {
    // If selecting 'end' date and 'start' is already set, disable dates before start
    if (datePickerMode === 'end' && filters.dateRange.start) {
  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        ...
        >
          <DayButtonInner isSelected={isSelected} isInRange={inRange}>
            <DayText isSelected={isSelected} isToday={isToday} isDisabled={disabled}>
              {day}
            </DayText>
          </DayButtonInner>
        </DayButton>
...
const DayButtonInner = styled.View<{ isSelected: boolean; isInRange: boolean }>`
  width: 100%;
  height: 100%;
  align-items: center;
  justify-content: center;
...
const DayText = styled.Text<{ isSelected: boolean; isToday: boolean; isDisabled: boolean }>`
  font-size: 14px;
  color: ${props =>
    props.isDisabled
      ? '#D1D5DB'
      ...
      : '#374151'};
  font-weight: ${props =>
    props.isSelected || props.isToday ? '600' : '400'};
`;
